...
class DDoS(gym.Env):
    ...
    def __init__(self, render_mode=None, mode='cedric', size=5):
        ...
        for agent in self.graph.agents:
            self.account[agent//self.compression] = init_balance

        # We have 2 actions, corresponding to "join", "stay out"
        self.action_space = spaces.Discrete(2)
        self.account = [spaces.Discrete(self.account_limit)] * (len(self.graph.agents)//self.compression+1)
    ...
    def step(self, invest_n, action_n):
        ...
        for agent in coalition:
            ...
            if self.mode == 'no credit':
                self.account[agent//self.compression] = int(self.account[agent//self.compression] + payoff)
            # cedric mode
            if self.mode == 'cedric':
                self.account[agent//self.compression] = int(self.account[agent//self.compression] + credit/samples)
