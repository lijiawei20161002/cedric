    ...
    for step in range(max_rounds):
        ...
        for agent in graph.agents:
            ...
            if exp_exp_tradeoff > epsilon:
                action_n[agent] = np.argmax(Q[agent//env.compression][tuple_to_num(state_n),:])
                if state_n[agent] > 0:
                    invest_n[agent] = np.argmax(QC[agent//env.compression][tuple_to_num(state_n),:])
                else:
                    invest_n[agent] = 0
            else:
                action_n[agent] = env.action_space.sample()
                if state_n[agent//env.compression] > 0:
                    invest_n[agent] = random.randint(0, state_n[agent])
                else:
                    invest_n[agent] = 0
            new_state_n, reward_n = env.step(invest_n, action_n)
            print(new_state_n, reward_n)
            Q[agent][tuple_to_num(state_n), action_n[agent]] = Q[agent//env.compression][tuple_to_num(state_n), action_n[agent]] + alpha*(reward_n[agent]+discount_factor*np.max(Q[agent//env.compression][tuple_to_num(new_state_n), :])-Q[agent//env.compression][state_n[agent//env.compression], action_n[agent]])
            total_training_rewards[agent] += reward_n[agent]
            state_n[agent//env.compression] = new_state_n[agent//env.compression]
